...
namespace PaulEngine
{
	...
	public:
		...
		virtual void SetData(SetDataParams dataParams, const bool preferMap = true) override;
		virtual void MultiSetData(std::vector<SetDataParams> multiDataParams, const bool preferMap = true) override;
	...
	protected:
		...
		void MultiBufferSubData(std::vector<SetDataParams> multiDataParams);
		...
		void MultiSetMappedData(std::vector<SetDataParams> multiDataParams);

		// needs to call glMemoryBarrier(GL_CLIENT_MAPPED_BUFFER_BARRIER_BIT) before subsequent commands will see the updated data
		void SetMappedDataPersistent(const void* data, size_t size, size_t offset);
		void MultiSetMappedDataPersistent(std::vector<SetDataParams> multiDataParams);

		void SetMappedDataCoherent(const void* data, size_t size, size_t offset);
		void MultiSetMappedDataCoherent(std::vector<SetDataParams> multiDataParams);
		...
		static std::unordered_map<int, void(OpenGLShaderStorageBuffer::*)(std::vector<ShaderStorageBuffer::SetDataParams>)> s_WriteFunctions;
