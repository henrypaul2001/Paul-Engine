#include "pepch.h"
#include "OpenGLFramebufferNew.h"
...
namespace PaulEngine
{
	namespace OpenGLFramebufferUtils
	{
		static GLenum FramebufferAttachPointToGLenum(FramebufferAttachmentPoint attachPoint)
		{
			switch (attachPoint)
			{
				...
				case FramebufferAttachmentPoint::DepthStencil: return GL_DEPTH_STENCIL;
			}
	...
	OpenGLFramebufferNew::OpenGLFramebufferNew(const FramebufferSpecification& spec, std::vector<Ref<FramebufferAttachment>> colourAttachments, Ref<FramebufferAttachment> depthAttachment) : m_RendererID(0), m_Spec(spec)
	{
		PE_PROFILE_FUNCTION();
	...
	OpenGLFramebufferNew::~OpenGLFramebufferNew()
	{
		glDeleteFramebuffers(1, &m_RendererID);
	}

	Ref<FramebufferAttachment> OpenGLFramebufferNew::GetAttachment(FramebufferAttachmentPoint attachPoint)
	{
		PE_PROFILE_FUNCTION();
	...
	bool OpenGLFramebufferNew::AddColourAttachment(Ref<FramebufferAttachment> colourAttachment)
	{
	...
	bool OpenGLFramebufferNew::SetDepthAttachment(Ref<FramebufferAttachment> depthAttachment)
	{
	...
	void OpenGLFramebufferNew::Bind()
	{
		glBindFramebuffer(GL_FRAMEBUFFER, m_RendererID);
	}

	void OpenGLFramebufferNew::Unbind()
	{
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
	}

	void OpenGLFramebufferNew::Resize(const uint32_t width, const uint32_t height)
	{
		PE_PROFILE_FUNCTION();
		if (width == 0 || height == 0) {
	...
	int OpenGLFramebufferNew::ReadPixel(FramebufferAttachmentPoint attachPoint, int x, int y)
	{
		PE_PROFILE_FUNCTION();
	...
	bool OpenGLFramebufferNew::operator==(const FramebufferNew* other) const
	{
		if (!other) { return false; }
		return m_RendererID == dynamic_cast<const OpenGLFramebufferNew*>(other)->m_RendererID;
	}
