...
namespace PaulEngine
{
	...
	void EditorCamera::OnUpdate(const Timestep ts, const bool blockMovement)
	{
		...
		if (Input::IsKeyPressed(PE_KEY_LEFT_ALT)) {
			...
			if (Input::IsMouseButtonPressed(PE_MOUSE_BUTTON_MIDDLE)) {
				MousePan(delta);
			}
			else if (Input::IsMouseButtonPressed(PE_MOUSE_BUTTON_LEFT)) {
				MouseRotate(delta);
			}
			else if (Input::IsMouseButtonPressed(PE_MOUSE_BUTTON_RIGHT)) {
				MouseZoom(delta.y);
			}
		}
	...
	void EditorCamera::OnEvent(Event& e)
	{
		...
		dispatcher.DispatchEvent<MouseScrolledEvent>(PE_BIND_EVENT_FN(EditorCamera::OnMouseScroll));
	}
	...
	{
		// m_Yaw = m_Pitch = 0.0f; // Lock the cameras rotation
	...
	std::pair<float, float> EditorCamera::PanSpeed() const
	{
		...
		float xFactor = 0.0366f * (x * x) - 0.1778f * 0.3021f;
