...
namespace  PaulEngine {
	...
	public:
		...
		inline static void SetViewport(const glm::uvec2& position, const glm::uvec2& size) {
		...
		inline static void MultiDrawIndexedIndirect(const uint32_t count, const uint32_t offset = 0, const size_t stride = 0) {
			s_RenderAPI->MultiDrawIndexedIndirect(count, offset, stride);
		}

		inline static void MultiDrawIndexedIndirect(const uint32_t count, const uint32_t offset, const size_t stride, const DrawPrimitive drawPrimitive) {
			s_RenderAPI->MultiDrawIndexedIndirect(count, offset, stride, drawPrimitive);
		}
