...
namespace Engine {
	...
	public:
		...
		void SetWorldUp(const glm::vec3& newUp) { this->worldUp = newUp; UpdateCameraVectors(); }

		void SetYaw(const float newYaw) { this->yaw = newYaw; UpdateCameraVectors(); }
		void SetPitch(const float newPitch) { this->pitch = newPitch; UpdateCameraVectors(); }
		...
		void SetZoom(const float newZoom) { this->zoom = newZoom; }
		void SetNearClip(const float newClip) { this->nearClip = newClip; }
		void SetFarClip(const float newClip) { this->farClip = newClip; }
