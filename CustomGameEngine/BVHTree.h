...
namespace Engine {
	...
	private:
		...
		std::vector<std::pair<glm::vec3, Mesh*>> SortMeshesOnAxis(const std::vector<std::pair<glm::vec3, Mesh*>>& unsortedObjects, unsigned int axisIndex) {
			...
			switch (axisIndex) {
			case 0:
				std::sort(sorted.begin(), sorted.end(), StaticComparePositionX);
				break;
			case 1:
				std::sort(sorted.begin(), sorted.end(), StaticComparePositionY);
				break;
			case 2:
				std::sort(sorted.begin(), sorted.end(), StaticComparePositionZ);
				break;
			default:
				break;
			}
