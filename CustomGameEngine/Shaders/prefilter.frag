...
uniform unsigned int faceWidth;
uniform unsigned int faceHeight;
...
vec3 ImportanceSampleGGX(vec2 Xi, vec3 N, float roughness) {
	...
	// spherical coordinates to catersian coordinates - halfway vector
	vec3 H;
	H.x = cos(phi) * sinTheta;
	H.y = sin(phi) * sinTheta;
	H.z = cosTheta;

	// tangent space H to world space sample vector
